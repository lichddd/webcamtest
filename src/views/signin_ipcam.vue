...
<div class="signin">
  <div class="leftpart">
    <video :src="videoUrl" width="400" height="300" style="display:none"></video>
    ...
    <el-form  label-width="100px" style="margin-top:10px;">
      ...
      <el-form-item :label="language.signin_ipcam.now_select_user">
        <el-select v-model="selectuser" filterable :placeholder="language.signin_ipcam.please_select">
          <el-option v-for="(item,index) in userlist" :key="index" :label="item.name" :value="index">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item :label="language.signin_ipcam.learning">
        <el-switch v-model="islearning"  v-vue-tooltip="islearning?language.signin_ipcam.close:language.signin_ipcam.open" @change="changeLearning" on-text="" off-text="">
        </el-switch>
      </el-form-item>
      <el-form-item label="">
        <el-button @click="showTSNE()">{{language.signin_ipcam.chart}}</el-button>
  ...
  <div class="imglist">
    <div v-if="u.new" class="imglist-user" v-for="(u,index) in userlist">
      <h6>{{u.name}}  ({{u.imgs.length}})<i v-vue-tooltip="u.expend?language.signin_ipcam.collapse:language.signin_ipcam.expend" @click="u.expend=!u.expend" class="expendicon el-icon-arrow-down" :class="{'el-icon-arrow-up':u.expend}"></i></h6>
      <div class="imgs"  v-show="u.expend">
        <div v-for="(m,idx) in u.imgs" class="img">
          <img :src="m.image" />
          <el-popover width="250" class="setting" v-vue-tooltip="language.signin_ipcam.changepic">
            <div>
              <el-select v-model="editPersonStr" :placeholder="language.signin_ipcam.please_select_user">
          ...
          <el-button v-vue-tooltip="language.signin_ipcam.delete" type="text" icon="delete" class="delete" size="large" @click="removeImage(u.imgs,idx)"></el-button>
        </div>
      </div>
    </div>
    <div v-if="!u.new" class="imglist-user" v-for="(u,index) in userlist">
      ...
      <div class="imgs"  v-show="u.expend">
        <div v-for="(m,idx) in u.imgs" class="img">
          ...
          <el-popover width="250" class="setting"  v-vue-tooltip="language.signin_ipcam.changepic">
            <div>
              <el-select v-model="editPersonStr" filterable :placeholder="language.signin_ipcam.please_select_user">
                <el-option v-if="iii!=index" v-for="(item,iii) in userlist" :key="iii" :label="item.name" :value="iii">
                </el-option>
              </el-select>
              <el-button type="text" icon="check" style="padding:0px;    margin-left: 5px;" size="large" @click="changePerson(editPersonStr);"></el-button>
    ...
    addPerson() {
      ...
      for (var i = 0; i < this.userlist.length; i++) {
        if (this.userlist[i].name == this.new_person_name) {
          ...
          this.$message({
            showClose: true,
            message: this.language.signin_ipcam.user_exist,
            type: "error"
          });
          return;
        }
